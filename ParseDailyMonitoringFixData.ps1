...
function Get-ParsedDailyReport() {
    ...
    $date = $fileNameSplit[$fileNameSplit.Length - 2] + " " + $fileNameSplit[$fileNameSplit.Length - 1].Replace(".txt", "")

    if ($date.Contains("Dec")) {
        $date = $date + ", " + (Get-Date).AddYears(-1).Year
    }

    $date = [DateTime]::Parse($date).ToString("MM/dd/yyyy h:mm")

    #create a result array
    $resultArray = @()

    #skip the header and read the rows of the text
    for ($i = 9; $i -lt $fileInput.Count; $i++) {
...
$emailTexts = Get-ChildItem -Path .\fixData | Where {$_.Name -like "*.txt"}


foreach ($file in $emailTexts) {
    $output = Get-ParsedDailyReport -fileInput (Get-Content -Path $file.FullName) -fileName $file.Name

    $timeRan = $output.Run_Date | Select -First 1 | Out-String

    Write-Host $timeRan $file.Name
    ...
    try {
        ...
        Write-Host "Issue with" $file.Name -ForegroundColor Yellow
    }
}
